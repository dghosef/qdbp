...
_qdbp_bigint_t _qdbp_get_boxed_int(_qdbp_object_ptr obj) {
...
_qdbp_object_ptr _qdbp_boxed_int_unary_op(_qdbp_object_ptr arg0,
  ...
  _qdbp_bigint_t i = arg0->data._qdbp_boxed_int->value;
  _qdbp_drop(arg0, 1);
  switch (op) {
    case PRINT:
      printf("%lld\n", i);
...
static _qdbp_object_ptr boxed_binary_arith_op(_qdbp_object_ptr a,
  ...
  _qdbp_assert(op < EQ);
  _qdbp_bigint_t a_int = _qdbp_get_boxed_int(a);
  _qdbp_bigint_t b_int;
...
static _qdbp_object_ptr boxed_binary_cmp_op(_qdbp_object_ptr a,
  ...
  _qdbp_bigint_t a_int = _qdbp_get_boxed_int(a);
  if (_qdbp_is_unboxed_int(b)) {
    uint64_t b_int = _qdbp_get_unboxed_int(b);
    _qdbp_drop(b, 1);
    _qdbp_drop(a, 1);
#define MK_CMP_CASE(n, op) \
  case n:                  \
...
#undef MK_CMP_CASE
    ...
    _qdbp_bigint_t b_int = b->data._qdbp_boxed_int->value;
    _qdbp_drop(b, 1);
    _qdbp_drop(a, 1);
#define MK_CMP_CASE(n, op) \
  case n:                  \
...
#undef MK_CMP_CASE
    ...
    _qdbp_object_ptr b_val = _qdbp_invoke_1(b, VAL, b);
    _qdbp_assert(_qdbp_get_kind(b_val) == QDBP_INT);
    _qdbp_bigint_t b_int = *b_val->data.i;
    _qdbp_drop(b_val, 1);
    _qdbp_drop(a, 1);
#define MK_CMP_CASE(n, op) \
  case n:                  \
...
_qdbp_object_ptr _qdbp_box_unboxed_int_and_extend(_qdbp_object_ptr obj,
  ...
  _qdbp_init_field(&field, label,
                   ...
                   num_captures);
  _qdbp_label_add(&(ret->data._qdbp_boxed_int->other_labels), &field, 1);
  return ret;
}
...
_qdbp_object_ptr _qdbp_boxed_int_extend(_qdbp_object_ptr obj,
  ...
  if (_qdbp_is_unique(obj) && _QDBP_REUSE_ANALYSIS) {
    _qdbp_label_add(&obj->data._qdbp_boxed_int->other_labels, &field, 1);
    return obj;
  } else {
    _qdbp_object_ptr ret =
