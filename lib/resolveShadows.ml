let add_variable varnames name =
  ...
  AstTypes.IntMap.add name id varnames

let varname varnames name = AstTypes.IntMap.find name varnames
...
let resolve_shadows ast =
  ...
  let rec resolve_shadows varnames ast =
    ...
    | `MethodInvocation (receiver, (name, labelLoc), args, loc) ->
        let args =
          List.map
            (fun (name, arg, loc) -> (name, resolve_shadows varnames arg, loc))
            args
        in
    ...
    | `Abort loc -> `Abort loc
  in
  let ast = resolve_shadows AstTypes.IntMap.empty ast in
  ast
