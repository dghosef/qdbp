let build_import_map files ast =
  let rec build_import_map already_seen state ast =
    ...
    | `PatternMatch (obj, cases, _) ->
        ...
        in
        let imports =
          List.fold_left (build_import_map already_seen) state case_body_exprs
        in
        build_import_map already_seen imports obj
    | `Declaration (_, rhs, body, _) ->
        let imports = build_import_map already_seen state rhs in
        build_import_map already_seen imports body
    | `VariableLookup _ -> state
    | `StrProto _ -> state
    | `IntProto _ -> state
    | `Abort _ -> state
    | `Import (filename, loc) ->
        let imports, files = state in
        if AstTypes.StringSet.mem filename already_seen then
          Error.compile_error "cycle detected in imports" loc files
        else if not (AstTypes.StringMap.mem filename imports) then
          let files, import = ParserDriver.parse_file files filename in
          let already_seen = AstTypes.StringSet.add filename already_seen in
          let imports = AstTypes.StringMap.add filename import imports in
          build_import_map already_seen (imports, files) import
        else state
  in
  build_import_map AstTypes.StringSet.empty
    (AstTypes.StringMap.empty, files)
    ast
...
let resolve_imports imports ast =
  let rec resolve_imports ast =
    ...
    | `MethodInvocation (receiver, (name, labelLoc), args, loc) ->
        let args =
          List.map
            (fun (name, arg, loc) -> (name, resolve_imports arg, loc))
            args
        in
    ...
    | `Declaration ((name, nameLoc), rhs, body, loc) ->
        ...
        let body = resolve_imports body in
        `Declaration ((name, nameLoc), rhs, body, loc)
    | `VariableLookup (name, loc) -> `VariableLookup (name, loc)
    | `Import (filename, _) ->
        let import = AstTypes.StringMap.find filename imports in
