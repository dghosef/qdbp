...
let compile args =
  ...
  let files, ast =
    ParserDriver.parse_file AstTypes.StringMap.empty full_input_file
  in
  ...
  *)
  ...
  let ast = Refcount.refcount AstTypes.IntSet.empty AstTypes.IntSet.empty ast in
  ...
  let methods = AstTypes.IntMap.add main_method_id main_method methods in
