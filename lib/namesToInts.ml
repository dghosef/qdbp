let add_variable varnames name =
  ...
  AstTypes.StringMap.add name id varnames

let varname varnames name = AstTypes.StringMap.find name varnames
...
let names_to_ints ast =
  ...
  Hashtbl.add label_map "isAnInt:ThisArg" (Int64.of_int 64);
  Hashtbl.add label_map "Print:ThisArg" (Int64.of_int 69);
  Hashtbl.add label_map "+:ThisArg:Arg0" (Int64.of_int 84);
  Hashtbl.add label_map "-:ThisArg:Arg0" (Int64.of_int 139);
  Hashtbl.add label_map "*:ThisArg:Arg0" (Int64.of_int 140);
  Hashtbl.add label_map "/:ThisArg:Arg0" (Int64.of_int 193);
  Hashtbl.add label_map "%:ThisArg:Arg0" (Int64.of_int 254);
  Hashtbl.add label_map "=:ThisArg:Arg0" (Int64.of_int 306);
  Hashtbl.add label_map "!=:ThisArg:Arg0" (Int64.of_int 355);
  Hashtbl.add label_map "<:ThisArg:Arg0" (Int64.of_int 362);
  Hashtbl.add label_map ">:ThisArg:Arg0" (Int64.of_int 447);
  Hashtbl.add label_map "<=:ThisArg:Arg0" (Int64.of_int 455);
  Hashtbl.add label_map ">=:ThisArg:Arg0" (Int64.of_int 461);
  ...
  let rec names_to_ints varnames ast =
    ...
    | `PrototypeCopy
      ...
      ->
        ...
        let args =
          List.map (fun (name, loc) -> (varname varnames name, loc)) args
        in
        `PrototypeCopy
          ( ext,
            ((get_label name, labelLoc), (args, body, methLoc), fieldLoc),
            size,
            loc,
            op )
    ...
    | `MethodInvocation (receiver, (name, labelLoc), args, loc) ->
        let args =
          List.map
            (fun (name, arg, loc) -> (name, names_to_ints varnames arg, loc))
            args
        in
        ...
        let cases =
          List.map
            (fun ((name, nameLoc), ((arg, argLoc), body, patternLoc), loc) ->
              ...
              let body = names_to_ints varnames body in
              ( (get_tag name, nameLoc),
                ((varname varnames arg, argLoc), body, patternLoc),
                loc ))
            cases
        in
    ...
    | `Abort loc -> `Abort loc
  in
  let ast = names_to_ints AstTypes.StringMap.empty ast in
